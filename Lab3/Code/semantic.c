...
void addReadAndWriteFunc(){  
    if (DEBUG2) printf("addReadAndWriteFunc\n");
    ...
    read_type->u.function.defined_sign=true;
    read_type->u.function.line=-1;
    read_type->u.function.para_num=0;
    read_type->u.function.paras=NULL;
    read_type->u.function.ret=int_type;
    ...
    write_type->u.function.defined_sign=true;
    write_type->u.function.line=-1;
    write_type->u.function.para_num=1;
    FieldList write_func_arg=(FieldList)malloc(sizeof(struct FieldList_));  
    write_func_arg->name="WhoCares";
    write_func_arg->type=int_type;
    write_type->u.function.paras=write_func_arg;
    write_type->u.function.ret=int_type;
    write_func->type=write_type;

    insertTable(SymbolTable,read_func);
    insertTable(SymbolTable,write_func);
}

void semanticAnalyse(TreeNode root){  //程序的开始
    if (DEBUG2) printf("semanticAnalyse\n");
...
Type StructSpecifier(TreeNode node){
    if (DEBUG2) printf("StructSpecifier\n");
...
char* Tag(TreeNode node){
    if (DEBUG2) printf("Tag\n");
    assert(node->children_num==1);
    /* Tag -> ID */
    return semanticID(node->children[0]);
}

void VarDecFunc(TreeNode node,Type type,FieldList field){ // TODO:
    if (DEBUG2) printf("VarDecFunc\n");
...
void insertValList(FieldList field,FieldList f){
    if (DEBUG2) printf("insertValList\n");
...
bool inStructType(Type struct_type,char* name){  // 判断结构体中是否已经有同名域
    if (DEBUG2) printf("inStructType\n");
    FieldList p=struct_type->u.structure_tag;
    while(p!=NULL){
        if (DEBUG2) printf("%s",p->name);
...
void VarList(TreeNode node,FieldList field){ // TODO:
    if (DEBUG2) printf("VarList\n");
...
void DefListStruct(TreeNode node,Type type_struct){
    if (DEBUG2) printf("DefListStruct\n");
    /* DefList -> empty */
    if (node==NULL)
        return ;
    assert(node->children_num==2);
...
void DecListStruct(TreeNode node,Type type,Type type_struct){
    if (DEBUG2) printf("DecListStruct\n");
    assert(node->children_num==1 || node->children_num==3);
    /* DecList -> Dec */
    if (node->children_num==1){
...
Type Exp(TreeNode node){
    ...
    if (node->children_num==3 && strcmp(node->children[1]->name,"ASSIGNOP")==0){
        if (DEBUG2) printf("Exp ASSIGNOP EXP\n");
    ...
    /* Exp -> Exp RELOP Exp */
        ...
        if (DEBUG2) printf("Exp RELOP EXP\n");
        Type t1=Exp(node->children[0]);
        Type t2=Exp(node->children[2]);
        return t1;
    }
    ...
    /* Exp -> Exp MINUS Exp */
        ...
        if (DEBUG2) printf("Exp MINUS EXP\n");
        Type t1=Exp(node->children[0]);
        Type t2=Exp(node->children[2]);
        if (!typeEqual(t1,t2)){
            printErrorInfo(7,node->line,"error7"); // #7 操作数类型不匹配
    ...
    /* Exp -> Exp LB Exp RB */   // A[b]
        ...
        if (DEBUG2) printf("Exp LB EXP RB\n");
        Type t1=Exp(node->children[0]);
        if (t1->kind!=ARRAY){
...
bool valListEqual(FieldList f1,FieldList f2){  // 判读两个FieldList是否相同
    if (DEBUG2) printf("valListEqual\n");
    if (f1==NULL && f2==NULL){
        if (DEBUG2) printf("f1=f2=NULL\n");
        return true;
    }
    else if (f1==NULL){
        if (DEBUG2) printf("f1=NULL\n");
        return false;
    }
    else if (f2==NULL){
        if (DEBUG2) printf("f2=NULL\n");
        return false;
    }
    else{
...
void checkFunDec(HashNode symbol_table,char**all_func,int func_cnt){
    if (DEBUG2) printf("checkFunDec\n");
    for (int i=0;i<func_cnt;i++){
        if (DEBUG2) printf("#%d: %s\n",i,all_func[i]);
